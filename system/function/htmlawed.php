...
function htmLawed($t, $C = 1, $S = array())
{
	...
	$x = array_flip((isset($x[0]) && $x[0] == '*') ? explode('-', $x) : explode(',',
		$x . (!empty($C['safe']) ? ',on*' : '')));
	if (isset($x['on*'])) {
		unset($x['on*']);
		$x += array(
	...
	$x            = (isset($C['schemes'][2]) && strpos($C['schemes'],
			':')) ? strtolower($C['schemes']) : 'href: aim, feed, file, ftp, gopher, http, https, irc, mailto, news, nntp, sftp, ssh, telnet; *:file, http, https';
	$C['schemes'] = array();
	foreach (
	...
	$t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'hl_ent',
		str_replace('&', '&amp;', $t));
	...
	for ($i = -1, $ci = count($t); ++$i < $ci;) {
		...
		if (isset($x[0])) {
			if (strlen(trim($x)) && (($ql && isset($cB[$p])) or (isset($cB[$in]) && !$ql))) {
				...
				foreach (
					preg_split('`(\x01\x02[^\x01\x02]+\x02\x01)`', $x, -1,
						PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $v
				) {
...
function hl_ent($t)
{
	...
	if (($n = ctype_digit($t = substr($t, 1)) ? intval($t) : hexdec(substr($t,
			1))) < 9 or ($n > 13 && $n < 32) or $n == 11 or $n == 12 or ($n > 126 && $n < 160 && $n != 133) or ($n > 55295 && ($n < 57344 or ($n > 64975 && $n < 64992) or $n == 65534 or $n == 65535 or $n > 1114111))) {
...
function hl_prot($p, $c = null)
{
	...
	if (preg_match('`^([^:?[@!$()*,=/\'\]]+?)(:|&#(58|x3a);|%3a|\\\\0{0,4}3a).`i', $p,
			$m) && !isset($c[strtolower($m[1])])) { // Denied prot
		return "{$b}{$d}{$p}{$a}";
	}
	if ($C['abs_url']) {
...
function hl_spec($t)
{
	...
	$t = str_replace(array(
		...
		"\n",
		' ',
	), '', preg_replace_callback('/"(?>(`.|[^"])*)"/sm', create_function('$m',
...
function hl_tag($t)
{
	...
	static $aN = array(
		...
		'href'            => array('a' => 1, 'area' => 1),
		'hreflang'        => array('a' => 1),
		'hspace'          => array('applet' => 1, 'img' => 1, 'object' => 1),
		'ismap'           => array('img' => 1, 'input' => 1),
		'label'           => array('option' => 1, 'optgroup' => 1),
		...
		'onchange'        => array('input' => 1, 'select' => 1, 'textarea' => 1),
		'onfocus'         => array(
			'a'        => 1,
			'area'     => 1,
			'button'   => 1,
		...
		'onselect'        => array('input' => 1, 'textarea' => 1),
		...
		'src'             => array('embed' => 1, 'script' => 1, 'input' => 1, 'iframe' => 1, 'img' => 1),
		...
		'value'           => array('input' => 1, 'option' => 1, 'param' => 1, 'button' => 1, 'li' => 1),
		'valuetype'       => array('param' => 1),
		'vspace'          => array('applet' => 1, 'img' => 1, 'object' => 1),
		'width'           => array(
			'embed'    => 1,
			'hr'       => 1,
			'iframe'   => 1,
		...
		'wmode'           => array('embed' => 1),
		'xml:space'       => array('pre' => 1, 'script' => 1, 'style' => 1),
	); // Ele-specific
	static $aNE = array(
		'allowfullscreen' => 1,
		'checked'         => 1,
	...
	static $aNU = array(
		...
		'id'          => array('script' => 1),
		'lang'        => array('applet' => 1, 'br' => 1, 'iframe' => 1, 'param' => 1, 'script' => 1),
		'xml:lang'    => array('applet' => 1, 'br' => 1, 'iframe' => 1, 'param' => 1, 'script' => 1),
		'onclick'     => array(
			'applet'  => 1,
			'bdo'     => 1,
			'br'      => 1,
		...
		'style'       => array('param' => 1, 'script' => 1),
		'title'       => array('param' => 1, 'script' => 1),
	); // Univ & exceptions
	if ($C['lc_std_val']) {
	...
	foreach ($aA as $k => $v) {
		if (((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e]))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
			...
			if ($k == 'style' && !$C['style_pass']) {
				...
				$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS',
					'hl_prot', $v);
				$v = !$C['css_expression'] ? preg_replace('`expression`i', ' ',
					preg_replace('`\\\\\S|(/|(%2f))(\*|(%2a))`i', ' ', $v)) : $v;
	...
	if ($C['unique_ids'] && isset($a['id'])) {
		if (!preg_match('`^[A-Za-z][A-Za-z0-9_\-.:]*$`',
				($id = $a['id'])) or (isset($GLOBALS['hl_Ids'][$id]) && $C['unique_ids'] == 1)) {
			unset($a['id']);
		} else {
...
function hl_tag2(&$e, &$a, $t = 1)
{
	...
	if ($e == 'font') {
		...
		while (preg_match('`(^|\s)(color|size)\s*=\s*(\'|")?(.+?)(\\3|\s|$)`i', $a, $m)) {
			$a  = str_replace($m[0], ' ', $a);
			$a2 .= strtolower($m[2]) == 'color' ? (' color: ' . str_replace('"', '\'',
					trim($m[4])) . ';') : (isset($fs[($m = trim($m[4]))]) ? ($a2 .= ' font-size: ' . str_replace('"',
					'\'', $fs[$m]) . ';') : '');
		}
		while (preg_match('`(^|\s)face\s*=\s*(\'|")?([^=]+?)\\2`i', $a, $m) or preg_match('`(^|\s)face\s*=(\s*)(\S+)`i',
				$a, $m)) {
			$a  = str_replace($m[0], ' ', $a);
