...
Dialog.prototype = {
    ...
    'init': function () {
        ...
        this.dom.close_button = $('<span class="dialog_close_button"><svg version="1.1" id="close_btn" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20.11px" height="20.11px" viewBox="0 0 20.11 20.11" style="enable-background:new 0 0 20.11 20.11;" xml:space="preserve"><g><path style="fill:#333333;" d="M10.05,1.3c4.83,0,8.75,3.93,8.75,8.75s-3.93,8.75-8.75,8.75S1.3,14.88,1.3,10.05 S5.23,1.3,10.05,1.3 M10.05,0C4.5,0,0,4.5,0,10.05c0,5.55,4.5,10.05,10.05,10.05c5.55,0,10.05-4.5,10.05-10.05 C20.11,4.5,15.61,0,10.05,0L10.05,0z"/><line style="fill:none;stroke:#333333;stroke-width:1.3;stroke-miterlimit:10;" x1="5.93" y1="5.93" x2="14.15" y2="14.15"/><line style="fill:none;stroke:#333333;stroke-width:1.3;stroke-miterlimit:10;" x1="14.15" y1="5.93" x2="5.93" y2="14.15"/></g></svg></span>').get(0);
    ...
    'createContents': function (type, options) {
        ...
        switch (type) {
            case 'ajax':
                ...
                _html = this.createContents('loading', {'text': 'Загрузка...'});
                break;
            case 'ajax_notice':
                ...
                break;
            case 'loading':
                _html = '<div class="dialog_loading vmiddle"><div class="spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div><div class="dialog_loading_text">' + options.text + '</div></div>';
                status = 'loading';
                break;
            case 'message':
                var type = 'notice';
                ...
                switch (type) {
                    ...
                    case 'warning':
                        ...
                        break;
                    case 'confirm':
                        var yes_button_name = "Да";
                        var no_button_name = "Нет";
...
function Ajax(recvType, waitId) {
    ...
    aj.loading = 'Пожалуйста, подождите...';
        ...
        if (aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
            ...
            if (aj.recvType == 'HTML') {
                ...
                if (!aj.XMLHttpRequest.responseXML || !aj.XMLHttpRequest.responseXML.lastChild || aj.XMLHttpRequest.responseXML.lastChild.localName == 'parsererror') {
                    aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">Внутренняя ошибка, не возможно отобразить контент</a>', aj);
                } else {
...
window.ajaxget = function ajaxget(url, showid, waitid, loading, display, recall) {
...
window.ajaxpost = function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    s = 'Внутренняя ошибка, невозможно отобразить информацию';
                }
            }
        }
    ...
    var url_type = action[action.length - 1];
    if (url_type == '/') {
...
function showloading(display, waiting) {
    ...
    var waiting = waiting ? waiting : 'Пожалуйста, подождите...';
...
window.hideMenu = function hideMenu(attr, mtype) {
...
window.showDialog = function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
    if (mode == 'js') {
    ...
    if (closetime) {
        leftmsg = closetime + ' секунды до закрытия окна';
        showDialogST = setTimeout(closefunc, closetime * 1000);
        showconfirm = 0;
    }
    locationtime = isUndefined(locationtime) ? '' : locationtime;
    if (locationtime) {
        leftmsg = locationtime + ' секунды до перехода';
        showDialogST = setTimeout(closefunc, locationtime * 1000);
        showconfirm = 0;
    }
    confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
    canceltxt = canceltxt ? canceltxt : 'Нет';
    ...
    s += t ? t : '<span class="dialog_title_icon">Сообщение</span>';
    s += '</span><span class="dialog_close_button" id="fwin_dialog_close" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="Закрыть">&times;</span></h3>';
    if (mode == 'info') {
        s += msg ? msg : '';
    } else {
...
window.showSucc = function showSucc(msg) {
    var p = /<script[^\>]*?>([^\x00]*?)<\/script>/ig;
    msg = msg.replace(p, '');
    if (msg !== '') {
        showDialog(msg, 'succ', 'Сообщение', null, true, null, '', '', '', 3);
    }
};
...
window.showError = function showError(msg) {
    ...
    msg = msg.replace(p, '');
    if (msg !== '') {
        showDialog(msg, 'alert', 'Ошибка', null, true, null, '', '', '', 3);
    }
};

window.hideWindow = function hideWindow(k, all, clear) {
...
window.ajax_get_confirm = function ajax_get_confirm(msg, url) {
    if (msg != '') {
